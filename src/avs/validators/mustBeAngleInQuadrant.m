...
if ~isnumeric(Q) && ~islogical(Q)
    throwAsCaller(MException( ...
        "AV:validatorUsage:nonNumericOrLogicalInput", ...
        "Quadrant number must be numeric or logical."));
end

if ~isreal(Q)
    throwAsCaller(MException( ...
        "AV:validatorUsage:nonRealInput", ...
        "Quadrant number must be real."));
end

if ~isvector(Q)
    throwAsCaller(MException( ...
        "AV:validatorUsage:nonVectorInput", ...
...
if ~allfinite(Q) || ~all(Q == floor(Q), 'all')
    throwAsCaller(MException( ...
        "AV:validatorUsage:nonIntegerInput", ...
        "Quadrant number must be integer."));
end

if ~all(Q >= 1 & Q <= 4)
    throwAsCaller(MException( ...
        "AV:validatorUsage:outOfRangeInput", ...
...
elseif all(A >= -pi & A <= pi, "all")
    ...
    throwAsCaller(MException( ...
        "AV:validatorUsage:nonSameAngleWrap", ...
...
if ~any(inq)
    throwAsCaller(MException( ...
        "AV:validators:mustBeAngleInQuadrant", ...
