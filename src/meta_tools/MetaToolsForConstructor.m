    ...
    methods(Static)
        ...
        function namedArgStruct = add_handle_defaults(namedArgStruct, opt)
            ...
            for i = 1:length(propList)
                ...
                isPublic = strcmp(p.SetAccess, 'public');
                definedHere = strcmp(p.DefiningClass.Name, objClassName);
                hasDefault = p.HasDefault;
                isHandle = false;
                if hasDefault
                    isHandle = isa(p.DefaultValue, 'handle');
                end

                if isPublic && isHandle && hasDefault && definedHere
